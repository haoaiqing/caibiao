...
- (void)commonInit {
    ...
    _tagWidht = 80;
    // 默认的标签高度
    _tagHeight = 20*UIRate;
    // 第一个参数是左右两个标签之间的间隔  第二个参数是上下两个标签之间的间隔
    _tagPaddingSize = CGSizeMake(10*UIRate, 10*UIRate);
    // 第一个参数表示标签内部text文本左右距离文本框的距离
    _textPaddingSize = CGSizeMake(12.5*UIRate, 0);
    _fontTag = [UIFont systemFontOfSize:14*UIRate];
...
- (void)layoutTags {
    ...
    for (int i = 0; i < _tagButtons.count; i ++) {
        ...
        if ((offsetX + tagButton.frame.size.width + _tagPaddingSize.width) <= self.frame.size.width) {
...
- (void)deleteAllTags{
    ...
    for (UIButton *button in self.svContainer.subviews){
