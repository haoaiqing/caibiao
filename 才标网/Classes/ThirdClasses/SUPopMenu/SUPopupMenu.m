...
{
    if ([self.delegate respondsToSelector:@selector(dropMenuDidTappedAtIndex:)]) {
        [self.delegate dropMenuDidTappedAtIndex:btn.tag];
    }
    
    if (self.dropMenuDidTappedAtIndexBlock) {
        self.dropMenuDidTappedAtIndexBlock(btn.tag);
    }
